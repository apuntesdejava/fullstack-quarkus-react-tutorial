...
Todo construido sobre **un solo proyecto**, de forma incremental, episodio a episodio.
...
## 🏗️ El Proyecto: QuarkBoard

**QuarkBoard** es un sistema de gestión de tareas colaborativo en tiempo real, estilo Jira simplificado.
...
### ¿Cómo se ve QuarkBoard?

```
┌─────────────────────────────────────────────────────────────────┐
│  QuarkBoard                              👤 Diego Villanueva ▾  │
├──────────────┬──────────────┬───────────────────────────────────┤
...
│   │   ├── main/
│   │   │   ├── java/dev/quarkboard/
│   │   │   │   ├── domain/           # Entidades, VOs, casos de uso
│   │   │   │   │   ├── model/
...
│   │   │   │   └── infrastructure/   # Config, exceptions, mappers
│   │   │   └── resources/
...
│   │   └── docker-compose.o11y.yml   # Stack de observabilidad
│   ├── azure/
│   │   ├── 01-create-infra.sh        # Crea todos los recursos
│   │   ├── 02-deploy-apps.sh         # Despliega los contenedores
│   │   ├── 03-configure-secrets.sh   # Configura Key Vault
│   │   └── 99-destroy.sh             # ⚠️ Borra TODO para no gastar
│   └── terraform/                    # Bonus: IaC declarativa
│       ├── main.tf
│       └── modules/
│
...
> 💡 **Navegación por episodios:** cada episodio tiene su rama `ep/XX-nombre` y un tag `ep-XX`. Puedes hacer `git checkout ep-XX` para ver el estado exacto del código al final de ese episodio.
...
```
✦ JDK 21 (recomendado: Eclipse Temurin o GraalVM CE)
...
| # | Episodio |
|---|---|
| 02 | Arquitectura Hexagonal: teoría y estructura de paquetes |
...
| # | Episodio |
|---|---|
| 28.5 | OpenTelemetry + Grafana: métricas, trazas y logs en un solo lugar |
...
| 33 | Azure: desplegando QuarkBoard completo |
...
- ⚠️ **Al terminar cada episodio cloud**, ejecuta el script de limpieza:
  ...
  ```
  Este script elimina **todos los recursos** de QuarkBoard en segundos.
...
| Recurso | Link |
|---|---|
| 📁 Repositorio GitHub | _[proximamente]_ |
| 🎥 Canal YouTube | _[proximamente]_ |
| 💬 Discusiones | GitHub Discussions del repo |
...
Instalaremos y configuraremos todas las herramientas, crearemos el proyecto base de Quarkus con la estructura hexagonal inicial, levantaremos el stack de Docker y verificaremos que todo funciona correctamente.
